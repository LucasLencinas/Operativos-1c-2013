...
t_stream *serializar_posiciones(t_recurso * estructura) {
	...
	int size = 0, offset = 0;
	char * string = malloc(sizeof(uint8_t)*2);
	memcpy(string,&estructura->recx, size = sizeof(uint8_t));
	offset = size;
	memcpy(string + offset,&estructura->recy,size = sizeof(uint8_t));
...
t_nivelConCulpables * desserializarNivelConCulpalbes (char* pres){
	int offset = 0, tmp_size = 0;
...
t_asignacionRecurso * desserializarAsignacionRecurso(char* unRecurso){
	...
	memcpy(rec->IDunRecurso, rec+offset, tmp_size);
	return rec;
}
